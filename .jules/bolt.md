...
## 2026-05-16 - [Performance] Offload CPU-Intensive String Operations in MigrateClosedLoop
**Learning:** Performing sequential string manipulation (`StartsWith`, `Replace`) inside a `foreach` loop that iterates over a large set of tracked entities (e.g., thousands of photos) can cause measurable CPU-bound delays on the main thread, especially when combined with EF Core tracking assignments. Additionally, modifying `DbContext` tracked entities directly within concurrent tasks is not thread-safe.
**Action:** Offloaded the CPU-intensive string computations to the existing `Task.Run` concurrent block by pre-computing `newUrl` and returning it alongside other task results. The actual assignment to the tracked `photo.Url` is then performed safely on the main thread after `await Task.WhenAll()`. This overlaps execution without compromising Entity Framework's thread safety.
## 2026-05-18 - Backend Pagination without Auth Regressions
**Learning:** Adding proxy endpoints (e.g., `/api/photos/s3/{{id}}`) to defer S3 URL generation introduces severe authorization regressions because standard `<img>` tags don't attach JWT headers. While the frontend uses `?access_token` tricks, managing N+1 database queries for auth checks on lazy-loaded images creates a new DDOS bottleneck. Furthermore, adding pagination to an API breaks client-side filtering completely if the client only receives 50 items.
**Action:** Always prefer optimizing the size of the payload (via server-side pagination) while retaining batch-processed concurrent tasks (`Task.WhenAll`) rather than converting bulk operations into individual N+1 endpoint calls.
