...
namespace PhotoAppApi.Tests.Controllers
{
    public class AdminControllerTests
    {
        ...
        [Fact]
        public async Task GetReports_ReturnsAllReports_JoinedWithPhotos()
        {
            // Arrange
            using var context = GetDbContext();

            var photo1 = new Photo { Id = 1, Url = "http://example.com/photo1.jpg", UploaderUsername = "user1", FileName = "photo1.jpg" };
            var photo2 = new Photo { Id = 2, Url = "http://example.com/photo2.jpg", UploaderUsername = "user2", FileName = "photo2.jpg" };

            var report1 = new ImageReport { Id = 1, PhotoId = 1, Reason = "Spam", Status = "Pending", ReportedAt = System.DateTime.UtcNow.AddMinutes(-10) };
            var report2 = new ImageReport { Id = 2, PhotoId = 2, Reason = "Inappropriate", Status = "Pending", ReportedAt = System.DateTime.UtcNow };

            context.Photos.Add(photo1);
            context.Photos.Add(photo2);
            context.ImageReports.Add(report1);
            context.ImageReports.Add(report2);
            await context.SaveChangesAsync();
            ...
            Assert.Equal(2, firstReport.GetType().GetProperty("ReportId")!.GetValue(firstReport));
            Assert.Equal(2, firstReport.GetType().GetProperty("PhotoId")!.GetValue(firstReport));
            Assert.Equal("http://example.com/photo2.jpg", firstReport.GetType().GetProperty("PhotoUrl")!.GetValue(firstReport));
            Assert.Equal("user2", firstReport.GetType().GetProperty("Uploader")!.GetValue(firstReport));
            Assert.Equal("Inappropriate", firstReport.GetType().GetProperty("Reason")!.GetValue(firstReport));

            var secondReport = reportsList[1];
            Assert.Equal(1, secondReport.GetType().GetProperty("ReportId")!.GetValue(secondReport));
        }
        ...
        public async Task DeleteReport_WithValidId_UpdatesStatusToProcessed()
        {
            // Arrange
            using var context = GetDbContext();
            var report = new ImageReport { Id = 1, PhotoId = 1, Reason = "Spam", Status = "Pending" };
        ...
        public async Task DeleteReport_WhenExceptionOccurs_ReturnsStatusCode500()
        {
            ...
            // Add valid report, but close context
            var report = new ImageReport { Id = 1, PhotoId = 1, Reason = "Spam", Status = "Pending" };
            context.ImageReports.Add(report);
            await context.SaveChangesAsync();
            context.Dispose();
