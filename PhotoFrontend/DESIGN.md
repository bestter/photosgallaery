...
- **Modals / overlays:** Dialog chrome is `role="dialog"` + `aria-modal="true"` with an i18n `aria-label`. Dismiss on pointer is a dedicated sibling `<button type="button" class="fixed inset-0 …" tabIndex={-1}>` (dim + `backdrop-blur-sm`), not an `onClick` on the dialog itself. The panel is a following `relative` sibling so it stacks above the backdrop without `stopPropagation`. Do not put `onKeyDown` / `role="document"` / `tabIndex` on the dialog or panel to fake click-outside. Escape-to-close is a `document` `keydown` listener (or the native `<dialog>` `cancel` event), never an inner `stopPropagation` on keydown. Close controls use `t("common.close")`. Reference implementations: `InviteModal`, `GroupRequestModal`, `ReportModal`, Gallery upload overlay.
